...
**MUST** — Anclas inmutables y versionadas, tres selectores redundantes (posición, cita, contexto), texto canónico propio, reanclaje acotado por bloque, y **huérfano visible** cuando el reanclaje no tiene confianza suficiente. Toda migración de ancla entre versiones registra una confianza explícita; por debajo del umbral **0.80** (`08_RULES` R-030) el ancla queda huérfana visible, nunca se migra "por las dudas"; ante empate entre dos candidatos, huérfana. El barrido de `03_SPIKE_anclas.md §2`/§6 descarta los extremos (0.6 y 0.9) pero no distingue 0.75 de 0.8; 0.80 es la elección de criterio, dentro de ese rango, que este principio pide ante evidencia empatada — más margen contra la migración silenciosa, no menos —, no un número que el barrido produzca por sí solo.
...
| 2026-09-08 | P-05 | Corregida la justificación de `0.80`: el barrido descarta 0.6 y 0.9 pero no distingue 0.75 de 0.8; 0.80 es decisión de criterio dentro de ese rango, no resultado del barrido | El barrido (`spikes/r03/barrido_resultado.json`) da celdas idénticas para 0.75 y 0.8 en las tres semillas; la redacción anterior sugería un desempate numérico inexistente |
